...
# ---------------------------------------------------------
# UPDATE FRED COLLECTOR (Normalized Time)
# ---------------------------------------------------------
...
def update_fred_raw():
    ...
    # CHANGED: Added .normalize() to strip time components
    dates = pd.date_range(end=pd.Timestamp.today().normalize(), periods=2000, freq='D')
    data = {}
    
    # Synthetic Data Generation
    data["RRP_Balance"] = np.random.normal(2000, 100, 2000)
    ...
    log.info("FRED data updated (Synthetic - Normalized).")
    return df
"""

# ---------------------------------------------------------
# UPDATE CRYPTO COLLECTOR (Normalized Time)
# ---------------------------------------------------------
...
def update_crypto_raw():
    ...
    write_parquet(df, out_path)
    log.info("Crypto data updated (Synthetic - Normalized).")
    return df
"""
...
with open(base_path / "fred.py", "w") as f:
    f.write(fred_code)
print("Fixed fred.py timestamps")

with open(base_path / "crypto.py", "w") as f:
    f.write(crypto_code)
print("Fixed crypto.py timestamps")
