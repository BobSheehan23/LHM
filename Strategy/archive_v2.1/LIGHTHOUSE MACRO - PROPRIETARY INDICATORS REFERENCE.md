...
**Compiled From:** All published articles on [lighthousemacro.com](https://www.lighthousemacro.com) + 10-Pillar Macro Framework
...
1\. Macro Risk Index (MRI) - Updated 10-Pillar Formula

### SECTION 2: PILLAR COMPOSITE INDICES (NEW)
...
**Source:** 10-Pillar Macro Framework (January 2026)

### Formula (Updated)

```
MRI = 0.15 × (-LPI)           # Labor (inverted: low LPI = high risk)
    + 0.10 × PCI              # Prices (high = inflation risk)
    + 0.15 × (-GCI)           # Growth (inverted: low GCI = high risk)
    + 0.08 × (-HCI)           # Housing (inverted)
    + 0.10 × (-CCI)           # Consumer (inverted)
    + 0.10 × (-BCI)           # Business (inverted)
    + 0.07 × (-TCI)           # Trade (inverted)
...
- **Labor (15%):** Leading indicator, drives the cycle
- **Growth (15%):** Core GDP health, investment focus
- **Government (10%):** Fiscal dominance regime—the 2026 theme
- **Plumbing (10%):** System vulnerability, amplifies shocks
- **Consumer (10%):** 68% of GDP, confirms labor signals
- **Business (10%):** Transmission mechanism, capex decisions
- **Prices (10%):** Inflation risk, Fed reaction function
- **Housing (8%):** Interest rate sensitive, wealth effect
- **Trade (7%):** Global margin, tariff transmission
...
```python
# MASTER COMPOSITE
MRI = 0.15×(-LPI) + 0.10×PCI + 0.15×(-GCI) + 0.08×(-HCI) + 0.10×(-CCI)
    + 0.10×(-BCI) + 0.07×(-TCI) + 0.10×GCI-Gov + 0.05×(-FCI) + 0.10×(-LCI)
...
**Compiled by:** Claude (Anthropic) from Lighthouse Macro publications + 10-Pillar Framework
